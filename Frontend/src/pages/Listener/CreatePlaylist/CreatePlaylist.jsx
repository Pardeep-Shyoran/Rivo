...
const CreatePlaylist = () => {
  ...
  const searchMusics = async () => {
    ...
    try {
      setLoading(true);
      const response = await axiosInstance.get('/api/music/search/music', {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const res = await axiosInstance.post('/api/music/playlist', payload);
  ...
  if (loading && !searchMode) {
    return (
      <div className={styles.createPlaylist}>
        <Loader message="Loading music..." inline />
      </div>
    );
  }
    ...
    <div className={styles.createPlaylist}>
      <div className={styles.container}>
        <div className={styles.header}>
          <button className={styles.backBtn} onClick={() => navigate('/listener/dashboard')}>
            ← Back to Dashboard
          </button>
          <h1 className={styles.title}>Create Playlist</h1>
          <p className={styles.subtitle}>Build your own playlist by selecting tracks you love</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className={styles.form}>
          ...
          <div className={styles.formGroup}>
            <label className={styles.label}>
              Search & Select Tracks <span className={styles.optional}>({selectedMusics?.length || 0} selected)</span>
            </label>
            <div className={styles.searchBar}>
              <input
                ...
                onKeyPress={(e) => e.key === 'Enter' && (e.preventDefault(), searchMusics())}
                disabled={isSubmitting}
              />
              <button
                type="button"
                className={styles.searchBtn}
                onClick={searchMusics}
            ...
            {musics.length === 0 ? (
              <div className={styles.noMusic}>
                ...
                <p>{searchMode ? 'No tracks found matching your search' : 'No music tracks available yet'}</p>
              </div>
            ) : (
