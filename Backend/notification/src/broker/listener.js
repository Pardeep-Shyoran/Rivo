...
function startListener() {
    ...
    subscribeToQueue('user_logged_in', async (msg) => {
        const { email, fullName, timestamp, ip, ipAddress, userAgent } = msg;
    ...
    // Profile update security notification
        subscribeToQueue('user_profile_updated', async (msg) => {
            const { email, fullName, changed = [], timestamp, ip, ipAddress, userAgent } = msg;
            if (!changed.length) return; // Nothing notable changed
            const { subject, text, html } = templates.profileUpdated({ fullName, changed, timestamp, ip, ipAddress, userAgent });
            await sendEmail(email, subject, text, html);
        });
    ...
    // Password change security notification
        subscribeToQueue('user_password_changed', async (msg) => {
            const { email, fullName, timestamp, ip, ipAddress, userAgent } = msg;
            const { subject, text, html } = templates.passwordChanged({ fullName, timestamp, ip, ipAddress, userAgent });
            await sendEmail(email, subject, text, html);
        });
            ...
            subscribeToQueue('user_profile_picture_updated', async (msg) => {
                const { email, fullName, timestamp, ip, ipAddress, userAgent } = msg;
                const { subject, text, html } = templates.profilePhotoUpdated({ fullName, timestamp, ip, ipAddress, userAgent });
                await sendEmail(email, subject, text, html);
            });
            ...
            subscribeToQueue('user_profile_picture_deleted', async (msg) => {
                const { email, fullName, timestamp, ip, ipAddress, userAgent } = msg;
                const { subject, text, html } = templates.profilePhotoDeleted({ fullName, timestamp, ip, ipAddress, userAgent });
                await sendEmail(email, subject, text, html);
            });
